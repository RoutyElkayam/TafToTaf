  ...
  public class CalenderLogic
  {
    public static List<CalenderDto> SelectCalenderChild(int childId)//מקבלת קוד ילד ומחזירה את כל הטיפולים שלן
    {
      using (DAL.TafToTafEntities1 db = new DAL.TafToTafEntities1())
      {
        ...
        var Treatments = db.Calanders.Where(tret => tret.KindId == 1 && tret.DateStart.Value.Year==PublicLogic.CalcBeaginYear().Year&& tret.ChildId == childId).ToList();
    ...
    public static List<CalenderDto> SelectCalenderWorker(int workerId)//מקבלת קוד עובדת ומחזירה את כל הטיפולים שלה ואת הישיבות צוות והורים
    {
      using (DAL.TafToTafEntities1 db = new DAL.TafToTafEntities1())
      {
        ...
        List<CalenderDto> CalenderList = new List<CalenderDto>();
        var Treatments = db.Calanders.Where(tret => tret.KindId == 1 &&tret.DateStart.Value.Year==PublicLogic.CalcBeaginYear().Year &&tret.ProfessionalId == workerId).ToList();
    ...
    public static List<CalenderDto> SelectTeamMeeting(int workerId)//מפגשי צוות לעובדת
    {
      using (DAL.TafToTafEntities1 db = new DAL.TafToTafEntities1())
      {
        ...
        foreach (var knd in listKnd)
        {
          MeetingParents.AddRange(db.Calanders.Where(meet => meet.KindId == 2 && meet.DateStart.Value.Year == PublicLogic.CalcBeaginYear().Year
    ...
    public static List<CalenderDto> SelectChildParentMeeting(int childId)//מפגשי הורים לילד
    {
      using(DAL.TafToTafEntities1 db = new DAL.TafToTafEntities1())
      {
        ...
        var meetParent = db.Calanders.Where(meet => meet.KindId == 3 && meet.DateStart.Value.Year == PublicLogic.CalcBeaginYear().Year && meet.KinderGardenId==childknd.KindrGardenID).ToList();
